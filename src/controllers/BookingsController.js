...
class BookingsController {
  ...
  async getListBookingsByFilter(req, res) {
    try {
      const listbookings = await bookingsService.getListBookingsByFilter(req.query);
      if (listbookings.length === 0) return res.status(variable.NoContent).send();
      var obj = {
  ...
  async createBooking(req, res) {
    ...
    try {
      ...
      if (result === variable.ExceededBooking) {
        return res.status(variable.BadRequest).send("Each account can only book 5 bookings in the next 7 days");
      }
      if (result === variable.ConfirmBooking) {
        return res.status(variable.BadRequest).send("You already have a confirmed booking");
      }
      res.send(result);
    } catch (err) {
  ...
  async updateBooking(req, res) {
    try {
      ...
      if (serviceInsertArray) {
        serviceInsertArray = serviceInsertArray.map((item) => new Object({ serviceId: item }));
      }
      if (roleIdAuth != variable.ReceptionistRoleId && status != "Cancel")
        return res.status(variable.Forbidden).send("No permission! Update status only work for receptionist role!");
      if (req.body.date) {
      ...
      let update = await bookingsService.updateBooking(id, data, customerId, roleIdAuth);
      if (update === variable.Forbidden)
        return res.status(variable.Forbidden).send("No permission! Only update your booking");
  ...
  async updateManyStatuesBooking(req, res) {
    try {
      ...
      if (roleIdAuth != variable.ReceptionistRoleId && status)
        return res.status(variable.Forbidden).send("No permission! Only work for receptionist role!");
      let idArray = req.body.idArray;
      let update = await bookingsService.updateManyStatuesBooking(idArray, status);
      if (update === variable.Forbidden) return res.status(variable.Forbidden).send("No permission!");
      if (update === variable.NotFound) return res.status(variable.NotFound).send("No bookings is updated!");
      res.send(update);
    } catch (err) {
      res.status(variable.InternalServerError).send(err.message);
    }
  ...
  async deleteBooking(req, res) {
    try {
      ...
      if (delBooking === variable.Forbidden) return res.status(variable.Forbidden).send("No permission!");
  ...
  async deleteManyBookings(req, res) {
    try {
      ...
      if (delManyBookings == variable.NotFound) return res.status(variable.NotFound).send("No Bookings is deleted");
  ...
  async getBill(req, res) {
    try {
      ...
      let roleIdAuth = req.user.roleId;
      if (roleIdAuth != variable.ReceptionistRoleId && roleIdAuth != variable.AdminRoleId)
